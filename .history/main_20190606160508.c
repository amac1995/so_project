...
#include "jsmn.h"
//gcc -o server.o main.c -include my_protocol.h my_protocol.c jsmn.h jsmn.c
...
void * create_new_client(void *coon);
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		if (FD_ISSET(s, &rset))//check if something happen in the listenfd (new connection)
		{
			...
			if(pthread_create(pcoon, NULL, &create_new_client, &coon)){
